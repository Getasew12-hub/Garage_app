...
import { ArrowRight, Delete, Flag, Loader, Plus, Trash2, X } from 'lucide-react'
import serviceStore from '../store/service';

function Appointment() {
  const {GetUserAppointment,userAppointment,loading,GetUserCars,userCars,RemoveCars}=serviceStore();
...
GetUserAppointment();
  ...
  return (
    <div className='max-w-6xl mx-auto px-4 mt-20 space-y-30'>
      <div>
         <h2 className='font-bold mb-6 tracking-wider sm:text-2xl text-xl'>Your Appointments</h2>
...
<div className='flex  items-center gap-3 flex-wrap'>
              ...
              <span>status:</span><button  className={`flex gap-1.5 ${item.status=="in-progress" && "bg-yellow-500/30 text-yellow-500"} ${item.status=="completed" && "bg-green-500/30 text-green-500"} ${item.status=="canceled" && "bg-red-500/30 text-red-500"}  ${item.status=="booked" && "bg-blue-500/30 text-blue-500"} rounded py-1 px-2 cursor-pointer h-fit `}>{item.status}</button>
</div>
                </div>
            )
        })
      ...
      <p className='font-bold'>Your car</p>
      {
        userCars.map((item,index)=>{
            return(
                <div key={item._id} className='flex gap-6 backgroundDiv p-4 rounded w-fit my-4 max-sm:text-sm'>
                    ...
                    <button onClick={()=>setShowAddCarform(item._id)} className='flex gap-1.5 bg-blue-500 rounded py-1 px-2 cursor-pointer'>Book <ArrowRight/></button>

                    <button onClick={()=>RemoveCars(item._id)} ><Trash2 color='red' className='cursor-pointer ml-6'/></button>
                   { showAddCarform==item._id &&<div onClick={()=>setShowAddCarform(null)} className='fixed inset-0 flex justify-center items-center bg-black/50'>
                    <div className='bg-gray-700 mx-3 max-w-4xl relative w-full py-12 px-6 rounded-md' 
                    onClick={(e)=> e.stopPropagation()}>

                      <X className='absolute right-2.5 top-4 cursor-pointer' onClick={()=>setShowAddCarform(null)}/>
                   <SetNewAppointment formdata={item} setShowAppointmentform={setShowAddCarform}/>
