...
function MechanicWork() {
  ...
  const { GetMechanicsWork, loading, userAppointment, CompleteMechanicJob } =
    serviceStore();
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 my-20 space-y-10 ">
      ...
      {userAppointment.length === 0 && (
        <p className="text-center mt-10">No Jobs Assigned Yet</p>
      )}

      {userAppointment?.length > 0 && (
        <div className="max-h-full  overflow-auto  w-full overflowScroll ">
          <table className="   w-full text-sm  md:text-md  ">
            <thead className="sticky top-0 bg-gray-700 border border-gray-500 shadow-sm z-10   ">
              <tr>
                ...
                <th className="text-start p-1 px-2.5 text-nowrap">
                  Service Type
                </th>
            ...
            <tbody>
              {userAppointment.map((val, index) => (
                <tr className="border border-gray-500 " key={index}>
                  ...
                  <td className=" p-2.5">
                    {val?.appointment?.appointmentDate?.split("T")[0]}
                  </td>
                  <td className="p-2.5 text-nowrap">
                    {val?.appointment?.timeslot}
                  </td>

                  <td
                    className={`p-2.5 ${val?.status === "in-progress" && "text-yellow-500"} ${val?.status === "booked" && "text-blue-500"} ${val?.status === "cancled" && "text-red-500"} ${val?.status === "completed" && "text-green-500"}  `}
                  >
                    {val?.status}
                  </td>
                  ...
                  <td className=" p-2.5">
                    <div>
                      <button
                        ...
                        onClick={() => {
                          setShowMechanics(val?.mechanic);
                          setDis(val?.mechanicId[0]?.dis);
                        }}
                      >
                        Mechanics <ArrowRightIcon size={15} />
                      </button>
                    </div>
                  </td>
                  <td className=" p-2.5">
                    <div>
                      {val.status !== "completed" && (
                        <button
                          onClick={() =>
                            CompleteMechanicJob(val.appointment._id)
                          }
      ...
      {showMechanics.length > 0 && (
        <div
          className=" fixed inset-0 z-40 bg-black/25 flex justify-center items-start pt-20 "
          onClick={() => {
            setShowMechanics([]);
            setDis("");
          }}
        >
          <div
            className="backgroundDiv p-6 rounded relative"
              ...
              {showMechanics.map((mechanic, index) => (
                <div className="bg-gray-700 p-4  rounded" key={index}>
                  ...
                  {user._id === mechanic._id && (
                    <p className="text-sm mt-2 max-w-[230px]">{dis}</p>
                  )}
                </div>
              ))}
            </div>
