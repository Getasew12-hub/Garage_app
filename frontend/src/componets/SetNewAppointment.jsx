...
function SetNewAppointment({
  ...
  const { formLoading, SetAppointment, TimeSlot, UpdateAppointment } =
    serviceStore();
    ...
    async function getSlots() {
      if (appointment.appointmentDate && appointment.servicetype) {
        ...
        if (!appointment.timeslot) {
          setAppointment({
            ...appointment,
            timeslot: response.availableSlots[0]?.displayTime,
            fullTimeSlot: response.availableSlots[0]?.startTime,
          });
        }
      }
    }
  ...
  function handleInput(e) {
    if (e.target.name == "timeslot") {
      const fulltime = slots?.find(
        (slot) => slot.displayTime == e.target.value,
      );
      setAppointment({
        ...appointment,
        [e.target.name]: e.target.value,
        fullTimeSlot: fulltime.startTime,
      });
    } else {
  ...
  return (
    <div>
      <fieldset className="border border-gray-400 rounded p-4">
        <legend className="sm:text-xl text-md font-bold text-center ">
          {update ? "Update Appointment" : "New Appointment"}
        </legend>
        ...
        <form onSubmit={handleForm} className="space-y-5">
          ...
          <div className="flex justify-between gap-3 max-sm:flex-col">
            ...
            <div className="appointmentdiv">
              ...
              <input
                ...
                readOnly={update || formdata?.model}
              />
            </div>
            <div className="appointmentdiv">
          ...
          <div className="appointmentdiv">
            ...
            <input
              ...
              readOnly={update || formdata?.VIN}
            />
          </div>
          <button
            disabled={formLoading}
            type="submit"
            className={`${formLoading ? "cursor-not-allowed bg-green-400/50 " : "bg-green-400 "} py-2 px-3 rounded text-sm w-full cursor-pointer font-bold flex justify-center items-center`}
          >
            {formLoading ? (
              <Loader size={18} />
            ) : update ? (
