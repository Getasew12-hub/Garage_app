...
function SelectMechanics({ setShow, val, update = false }) {
  ...
  if (smalLoad)
    return (
      <div className="flex justify-center items-center h-screen w-screen fixed inset-0 bg-black/40 z-50">
  ...
  return (
    <div
      className="fixed inset-0  flex justify-center items-center bg-black/20 z-40 overflow-hidden"
      onClick={() => setShow(false)}
    >
      <div
        className="bg-gray-700 p-10 rounded max-w-4xl w-full"
        onClick={(e) => e.stopPropagation()}
      >
        <p className="font-bold md:text-2xl text-xl mb-5">
          Select Mechanics for this job
        </p>
        ...
        {MechanicsData?.length > 0 && (
          <div className="max-h-full overflow-y-auto  max-w-4xl w-full  ">
                  ...
                  <React.Fragment key={index}>
                    <tr className="border border-gray-500 ">
                      ...
                      <td className=" p-2.5">
                        <div>
                          <button
                            className="border h-6 w-6 cursor-pointer rounded flex justify-center items-center"
                            onClick={() => handleSelectMechanic(val._id)}
                          >
                            {selectedMechanic.includes(val._id) && (
                      ...
                      <td className=" p-2.5">
                        <div>
                          <button
                            onClick={() => handleDescriptionOpen(val._id)}
                            className="flex items-center gap-2 bg-gray-400 rounded-full py-1 px-2 cursor-pointer "
                          >
                            Description{" "}
                          </button>
                        </div>
                    ...
                    >
                      <td colSpan={4} className="w-full relative h-20">
                        <textarea
                          name="description"
                          placeholder="Description"
                          id="description"
                          className="border-x border-gray-500 w-full h-full p-2 resize-none  outline-0"
                          value={getMechanicData(val._id)?.dis || ""}
                          onChange={(e) =>
                            handleDescriptionSend(e.target.value, val._id)
                          }
                        ></textarea>
                      </td>
                    </tr>
            ...
            {val?.appointment?.status !== "completed" && (
              <button
                onClick={handFormSubmit}
                disabled={selectedMechanic.length == 0 || formLoading}
                className={`${selectedMechanic.length == 0 || formLoading ? "bg-blue-400/40 cursor-not-allowed" : "bg-blue-400 cursor-pointer"}  p-2 rounded font-semibold  float-end mt-6`}
              >
                {formLoading ? (
