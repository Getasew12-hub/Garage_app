...
function Navbar({setOpenAdminMenu,showLogin}) {
  ...
  return (
    ...
    <div >
    <div className={`${addminPath ? "bg-black" : "bg-black/50"} px-4 py-1 flex justify-between items-center fixed top-0 w-full md:pt-3 xl:px-20 z-50`}>

    {/* dachboard menue */}
   {addminPath && <Menu onClick={() => setOpenAdminMenu(true)} className='lg:hidden cursor-pointer  text-blue-400'/>}
        ...
        <div className='flex justify-center items-center cursor-pointer' onClick={() => navigate("/")} >
            ...
            <span className='font-bold text-white font-serif  text-xl hidden md:block'>Family Garage</span>
        </div>
        ...
        <div className='font-semibold text-white lg:flex hidden'>
         ...
         {!user ?  <button className='bg-blue-400 px-4 py-1 rounded-full cursor-pointer text-sm' onClick={()=>showLogin(true)}>Login</button>:
            <button className='bg-gray-400 px-4 py-1 rounded-full cursor-pointer text-sm' onClick={LogoutUser}>Logout</button>}

        </div>

        <Menu onClick={() => setOpenMenu(!openMenu)} className='lg:hidden cursor-pointer text-white'/>
    </div>
{/* mobile menu  */}

  <div ref={menuRef} className={`${openMenu ? "translate-x-0" : "translate-x-full"} transition-all duration-300  font-semibold text-white flex flex-col lg:hidden fixed top-0 right-0 w-2/3 max-w-96 h-screen bg-gray-700 px-4 py-1 pt-10 gap-4 z-50 border-l border-gray-400`}>
            <X className='absolute top-4 right-4 cursor-pointer w-fit' onClick={()=>setOpenMenu(false)}/>
              <Link to="/" className='mr-4 hover:text-blue-400 w-fit' onClick={handleMenu}>Home</Link>
         ...
         {!user ?  <button className='bg-blue-400 px-4 py-1 rounded-full cursor-pointer w-fit' onClick={()=>{
          showLogin(true);
          handleMenu();

         }} >Login</button>:
            <button className='bg-gray-400 px-4 py-1 rounded-full cursor-pointer w-fit' onClick={()=>{
          LogoutUser();
          handleMenu();
