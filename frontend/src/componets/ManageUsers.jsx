...
function ManageUsers() {
    const {loading,GetUsers,AppointmetData,UpdateUserRole,DeleteUser}=addminStore();
  ...
  return (
    <div>
      ...
      <h2 className="font-bold md:text-2xl text-xl mb-3 ">Manage Users </h2>
       <Search data={AppointmetData} searchResult={searchResult} type={"user"}/>
        </div>
              ...
              <table className="   w-full text-sm  md:text-md  ">
                <thead className="sticky top-0 bg-gray-700 border border-gray-500 shadow-sm z-10   ">
                  <tr>
                    ...
                    <th className="text-start p-1 px-2.5 max-md:hidden">Address</th>
                ...
                <tbody>
                  {services.map((val, index) => (
                    <tr className="border border-gray-500 " key={index}>
                      ...
                      <td className=" p-2.5 ">
                         <select name="action" className='border border-gray-500 py-1 px-2 rounded text-[10px] cursor-pointer' defaultValue={val.role} onChange={(e)=>UpdateUserRole(val._id,e.target.value)}>
                      ...
                      <td className=" p-2.5">
                        <div className='flex items-center gap-4'>
                          <button
                          onClick={()=>DeleteUser(val._id)} className='cursor-pointer'><Trash2 size={18} color='red'/></button>
