...
AddSerivceJob:async ({seletDate, val}) => {
    ...
    try {
        const response=await axios.post("/admin/addServiceJob",{seletDate,val});
        set({AppointmetData:get().AppointmetData.filter((vehicle) => vehicle._id !== val._id)});
        return "true"
    } catch (error) {
        console.error("Login error:", error);
        return "false"
...
UpdateAppointmentStatus:async (id,val) => {
    set({formLoading:true});
    try {
        const response=await axios.put(`/appointment/updateAppointment/${id}`,{status:val});
...
UpdateServiceMechanics:async ({seletDate, val}) => {
    set({formLoading:true});
    try {
        const response=await axios.put("/admin/updateServiceMechanics",{seletDate,val});
        set({AppointmetData:get().AppointmetData.map((appointment)=>{
...
DeleteServiceJob:async (id) => {
    ...
    try {
        ...
        set({AppointmetData:get().AppointmetData.filter((vehicle) => vehicle._id !== id)});
...
UpdateUserRole:async (id,val) => {
    set({formLoading:true});
    try {
        const response=await axios.put(`/admin/changeUserRole/${id}`,{role:val});
        set({AppointmetData:get().AppointmetData.map((appointment)=>{
            if(appointment._id===id){
